		...
		<client-only>
			<template #fallback>
				<div class="flex gap-2 h-12" style="direction: ltr">
					<div class="col-span-1 w-24 bg-neutral-100 rounded-2xl animate-pulse"></div>
					<div class="col-span-full bg-neutral-100 h-full flex-1 rounded-2xl animate-pulse"></div>
				</div>
			</template>
...
watch(
	modelValue,
	(v, oldValue) => {
		if (oldValue != v) if (typeof oldValue == typeof v || v) mobile.setValue(v, true);
	},
	{ immediate: true },
);
