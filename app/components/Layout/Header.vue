<template>
  <div class="fixed w-full z-50 px-4 md:px-0">
    <header
      class="container ltr:bg-gradient-to-tr backdrop-blur-sm rtl:bg-gradient-to-tl from-goldContrast to-primary/80 mx-auto dark:!to-primary-dark dark:!from-amber-950 my-2 md:my-4 p-2 md:p-3 rounded-lg shadow-xl transition-colors duration-300"
    >
      <div
        class="flex gap-2 md:gap-5 items-stretch justify-between ltr:flex-row-reverse"
      >
        <!-- this is a comment -->
          ...
          <ul class="flex mx-auto w-fit h-full gap-1 md:gap-2">
            <li v-for="(link, n) in links" :key="n">
              <NuxtLink
                ...
                class="text-white px-2 md:px-4 flex flex-col gap-1 items-center justify-center py-1 h-full hover:!text-yellow-300 after:w-0 after:h-1 after:bg-yellow-300 after:block after:absolute relative after:bottom-0 after:rounded-t-full after:transition-all after:duration-300 text-sm md:text-base"
              >
                {{ $t(link.title) }}
              </NuxtLink>
            </li>
        ...
        >
          ...
          <!-- Mobile Menu Button -->
          <v-btn @click="showMobileMenu = !showMobileMenu" size="small" class="min-w-0 p-2 sm:hidden">
