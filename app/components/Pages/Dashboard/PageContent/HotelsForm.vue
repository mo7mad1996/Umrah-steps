<template>
	<h3 class="between-lines text-gray-700 dark:text-gray-300 text-3xl my-6 text-center font-bold">
		{{ $t("dashboard.site_settings.hotels.title") }}
	</h3>
	...
	<section class="grid md:grid-cols-2 gap-6" v-if="PageContentStatus == 'success'">
		...
		>
			...
			>
				...
				<div class="mt-4">
					<InputsSubmit :title="$t('global.save')" class="w-full" :isLoading="isSubmitting" />
				</div>
			</Form>
		</div>
	</section>
...
const { data: pageContent, status: PageContentStatus } = useAsyncData("pageContent-hotels", () =>
	useApi()
...
const updatePageContent = async (values: any) => {
	try {
		await useApi().put("/page-content/hotels", values);
