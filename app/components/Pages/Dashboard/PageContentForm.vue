			...
			<div class="space-y-8">
				<section>
					<h3 class="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
						...
						{{ $t("dashboard.site_settings.seo_section") }}
					</h3>
					<div class="grid md:grid-cols-2 gap-4">
						<InputsText
							v-model="form.seo.title.ar"
							:title="$t('dashboard.site_settings.seo_title_ar')"
							name="seo.title.ar"
							icon="mdi:format-title"
						/>
						<InputsText
							v-model="form.seo.title.en"
							:title="$t('dashboard.site_settings.seo_title_en')"
							name="seo.title.en"
							icon="mdi:format-title"
						/>
						<InputsTextarea
				...
				<section>
					<h3 class="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
						<Icon name="mdi:text" />
						{{ $t("dashboard.site_settings.content_section") }}
					</h3>
					<div class="grid md:grid-cols-2 gap-4">
						<InputsText
				...
				<section>
					<h3 class="text-xl font-semibold mb-4 flex items-center gap-2 dark:text-white">
						<Icon name="mdi:image" />
						{{ $t("dashboard.site_settings.images_section") }}
					</h3>
					<div class="grid md:grid-cols-2 gap-4">
						<InputsFile
							ref="mainImage"
							...
							accept="image/*"
							name="content.image"
							:title="$t('dashboard.site_settings.main_image')"
						/>
						<InputsFile
							ref="images"
							accept="image/*"
							...
							multiple
							name="content.images"
							:title="$t('dashboard.site_settings.additional_images')"
						/>
					</div>
				</section>
...
const loadPageContent = async () => {
	loading.value = true;
	try {
		const response = await useApi().get(`page-content/${props.page}`);
...
const handleSubmit = async () => {
	...
	try {
		...
		if (images.value?.files?.length) {
			data.content.images = [...data.content.images, ...(await images.value.uploadFiles())];
		}

		const response = await useApi().put(`page-content/${props.page}`, data);

		if (response.data.success) {
			showToast(t("dashboard.site_settings.success_update"), "success");
		}
	} catch (error: any) {
		console.error("Error updating page content:", error);
		showToast(t("dashboard.site_settings.error"), "error");
	} finally {
