<template>
	<div class="container mx-auto px-4 md:px-6" v-if="contentStatus == 'success'">
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-4" v-if="status == 'pending'">
			<div class="bg-gray-300 animate-pulse h-64 md:h-80 rounded-3xl" v-for="n in 3" :key="n"></div>
		</div>
		...
		<section class="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8" v-if="status == 'success'">
			...
			<div class="lg:col-span-2">
				...
				<!-- buttons -->
				<div class="flex gap-2 items-center ms-auto w-fit rtl:flex-row-reverse p-2 mt-4">
					<button
...
const { data: hotels, status, error, refresh } = useHotels(ref({ per_page: 15 }));
