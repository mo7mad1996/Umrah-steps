<template>
	<div>
				...
				<v-table v-else class="dark:bg-gray-800">
					<thead>
						<tr>
							...
							<th class="text-start dark:text-white">{{ $t("hotels.cities.name_ar") }}</th>
							<th class="text-start dark:text-white">{{ $t("hotels.cities.name_en") }}</th>
		...
		<GlobalCityDialog v-model="dialogOpen" :city="selectedCity" @saved="handleSaved" />
...
const handleDelete = async () => {
	try {
		...
		showToast.success(t("cities.deleted"));
		deleteDialogOpen.value = false;
		cityToDelete.value = null;
	} catch (error: any) {
		showToast.error(error.message || t("cities.error"));
	}
};
