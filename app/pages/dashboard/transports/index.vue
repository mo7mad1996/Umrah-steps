        ...
        <div class="flex justify-between items-center py-6">
          <div>
            <h1 class="text-2xl font-bold text-gray-900">
              إدارة وسائل المواصلات
            </h1>
            <p class="mt-1 text-sm text-gray-500">
              إدارة وإضافة وتعديل وسائل المواصلات
            </p>
          </div>
          <button
      ...
      <div class="bg-white rounded-lg shadow-sm overflow-hidden">
        ...
        <div v-if="filteredTransports.length === 0" class="text-center py-12">
          ...
          <h3 class="mt-2 text-sm font-medium text-gray-900">
            لا توجد وسائل مواصلات
          </h3>
          <p class="mt-1 text-sm text-gray-500">
            ابدأ بإضافة وسيلة مواصلات جديدة
          </p>
        </div>
      </div>
    </div>
        ...
        <div class="mt-3">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-medium text-gray-900">
              {{
                showEditModal
                  ? "تعديل وسيلة المواصلات"
                  : "إضافة وسيلة مواصلات جديدة"
              }}
            </h3>
            <button
              @click="closeModal"
...
const filteredTransports = computed(() => {
  return transports.value.filter((transport) => {
    const matchesSearch =
      transport.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      transport.description
        .toLowerCase()
        .includes(searchQuery.value.toLowerCase());
    const matchesType =
      !selectedType.value || transport.type === selectedType.value;
    const matchesStatus =
      !selectedStatus.value || transport.status === selectedStatus.value;
...
usePageTitle("dashboard.transports");
